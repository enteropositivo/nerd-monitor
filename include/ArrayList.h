//- https://github.com/wachidsusilo/ArrayList
#ifndef ARRAY_LIST_H
#define ARRAY_LIST_H
...
public:
     ...
     *
     */
    ArrayList() : m_Data(NULL), m_Size(0), m_Capacity(0) {
        _reallocate(2);
    }
    ...
    bool addAll(const ArrayList& other) {
        if (m_Size + other.m_Size >= m_Capacity) {
            if (!_reallocate(m_Capacity + other.m_Capacity))
                return false;
        }
...
#if defined(ESP32) || defined(ESP8266)
    /**
     * @brief Check if the ArrayList contains an element that matches the
     * predicate.
     *
     * @param predicate is the predicate to match.
     * @return true if the element is in the ArrayList.
     ...
     * @param defaultValue is the default value to return if the index is out of
     * bounds.
...
#if defined(ESP32) || defined(ESP8266)
    /**
     * @brief Iterate over the ArrayList and call the predicate for each
     * element.
     *
    ...
    void sort(std::function<bool(T&, T&)> predicate) {
        for (size_t i = 1; i < m_Size; i++) {
            for (size_t j = i; j > 0 && predicate(m_Data[j - 1], m_Data[j]);
                 j--) {
                T tmp = m_Data[j - 1];
                m_Data[j - 1] = m_Data[j];
                m_Data[j] = tmp;
